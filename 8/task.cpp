...
void set_date_as_tm(std::string date_as_string, tm &last_performed_date){
    ...
    year += date_as_string[8];
    year += date_as_string[9];

    last_performed_date.tm_mday = stoi(day);
    last_performed_date.tm_mon= stoi(month);
    last_performed_date.tm_year = stoi(year);
}
...
std::istream& operator>>(std::istream& is, Task &task_to_add)
{
    ...
    */
    set_date_as_tm(result[2], last_performed_date);
    task_to_add.set_last_performed_month(last_performed_date.tm_mon);
    task_to_add.set_last_performed_year(last_performed_date.tm_year);
