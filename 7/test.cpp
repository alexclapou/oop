...
void get_list_of_saved_tasks__valid_input__list_of_saved_tasks_returned()
{
    ...
    index = 0;
    service.set_mode('B');
    Task task1("a", "1", date, 10, "a");
    service.add_to_servant_list(task1);
    servant_tasks.push_back(task1);
    Task task2("b", "1", date, 10, "a");
    service.add_to_servant_list(task2);
    servant_tasks.push_back(task2);
    Task task3("c", "1", date, 10, "a");
    service.add_to_servant_list(task3);
    servant_tasks.push_back(task3);
    std::vector<Task> saved_tasks = service.get_list_of_saved_tasks();
    for(Task task:saved_tasks){
        assert(task == servant_tasks[index]); 
        index++;
    }
}
...
void get_list_of_tasks__valid_input__list_of_task_returned()
{
    ...
    index = 0;
    service.set_mode('A');
    Task task1("a", "a", date, 10, "a");
    Task task2("b", "b", date, 10, "b");
    Task task3("c", "c", date, 10, "c");
    service.set_filepath("/home/alexclapou/fac/oop/folder cu spatii/test_file.txt");
    service.add("a", "a", date, 10, "a");
    tasks_saved.push_back(task1);
    service.add("b", "b", date, 10, "b");
    tasks_saved.push_back(task2);
    service.add("c", "c", date, 10, "c");
...
void get_number_of_tasks__valid_input__number_of_task_returned()
{
    ...
    service.set_mode('A');
    service.add("a", "a", date, 10, "a");
    service.add("b", "a", date, 10, "a");
    service.add("c", "a", date, 10, "a");
    service.add("d", "a", date, 10, "a");
...
void get_list_by_type_and_times_performed__valid_input__list_returned()
{
    ...
    service.add("a", "a", date, 10, "a");
    service.add("b", "a", date, 10, "a");
    service.add("c", "ac", date, 10, "a");
    service.set_mode('B');
    std::vector<Task> new_tasks = service.get_list_by_type_and_times_performed("a", 100);
...
void test_service()
{
    ...
    get_list_of_saved_tasks__valid_input__list_of_saved_tasks_returned();
